...
- **MLflow**
  ...
  - `MLFLOW_S3_ENDPOINT_URL=http://storage:9005`
...
- Set **server domains/host** so virtual-hosted requests can be resolved by RustFS:
  ...
  ```env
  RUSTFS_SERVER_DOMAINS=storage:9005
  ```
...
```sh
curl -s http://127.0.0.1:9005/health | grep -q '\"status\"\\s*:\\s*\"ok\"'
```
...
- Endpoint issues from MLflow → make sure `MLFLOW_S3_ENDPOINT_URL` uses the **service name** visible from MLflow (e.g., `http://storage:9005`).
